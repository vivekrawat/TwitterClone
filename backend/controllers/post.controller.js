...
exports.likePost = async (req, res, next) => {
    try {
        let postId = req.params.postId;
        let user = req.user;
        let responce = await Friendship.postLiked(user._id, { postId })//will make update in post-engagement model too
        if (responce.ok)
...
exports.repostPost = async (req, res, next) => {
    try {
        ...
        await Friendship.postReposted(user._id, { postId: post._id })
        res.json({
