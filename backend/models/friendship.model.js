...
/**
 * counts the number of users user is following
 * @param {*} user_id 
 * @returns 
 */
...
/**
 * checks if a user: user_id reposted a post: post_id
 * @param {*} user_id 
 * @param {*} post_id 
 * @returns 
 */
...
/**
 * operations when a post is liked
 * @param {*} user_id 
 * @param {*} post_id 
 * @returns 
 */
friendshipSchema.statics.postLiked = async function (user_id = null, post_id = null) {
    if (!post_id)
...
friendshipSchema.statics.postUnliked = async function (user_id = null, post_id = null) {
    if (!post_id)
        throw Error('Cannot determine post')
    let liked = await this.isLiked(user_id, post_id)
    if (!liked)
...
/**
 * operations when post is reposted
 * @param {*} user_id 
 * @param {*} param1 
 * @returns 
 */
friendshipSchema.statics.postReposted = async function (user_id = null, post_id = null) {
    if (!post_id)
        throw Error('Cannot determine post')
...
friendshipSchema.statics.postUnreposted = async function (user_id = null, post_id = null) {
    if (!post_id)
        throw Error('Cannot determine post')
    let reposted = await this.isReposted(user_id, post_id)
    if (!reposted)
